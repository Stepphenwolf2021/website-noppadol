...
## อัปเดต 3 (2026-07-14): Concept scheme (L1) — เชื่อมข้ามหมวดจริง
ปัญหาที่เจอ: การเชื่อมด้วย "หัวเรื่อง LCSH ตรงกันเป๊ะ" อย่างเดียวให้สะพานข้ามหมวดน้อยมาก (เหลือ 3 เส้น) และ
เคสเด่นอย่าง Hoffmann coffee ↔ fermentation ไม่เกิด เพราะ The World Atlas of Coffee ไม่มี LCSH subjects เลย
และหนังสือ fermentation อยู่หมวด Agriculture ทั้งคู่

แก้ด้วยการเพิ่มชั้น **concept scheme (SKOS)** ตาม blueprint L1 — ไฟล์ใหม่ `concepts-data.js`:
- รวมแนวคิดชื่อเดียวกันเป็นโหนดเดียว (cat "Coffee" กับ subject "Coffee" = โหนดเดียว ไม่ซ้ำอีก)
- เพิ่ม hub ข้ามหมวด: Beverages, Psychology & Mind, Society & Culture, Food & Fermentation
- เพิ่มเส้น `BROADER` (หัวเรื่องย่อย→กว้าง) + `RELATED_TO` (สะพานเชิงแนวคิด เช่น Coffee↔Food & Fermentation, Coffee↔Tea) รวม 18 เส้น
- วาดเป็น **เส้นประ** สีต่างจากเส้นหนังสือ + legend/gstats บอกจำนวน
- ผลลัพธ์: Hoffmann's World Atlas of Coffee → Coffee → (Beverages→Agriculture หรือ Food & Fermentation) → Noma Guide to Fermentation — **เชื่อมข้ามหมวดได้จริง**
- กราฟรวม 224 โหนด · 270 ความสัมพันธ์ (+18 เส้นแนวคิด)
- ตัดสินใจ: scheme นี้เป็น **curated static** สาธิตรูปแบบ ระบบจริง (L1) สร้างกึ่งอัตโนมัติจาก DDC/LCSH + LLM + ผู้ใช้ยืนยัน
- demo ยังไม่เชื่อม LLM/API ใด ๆ (static บน GitHub Pages) — AI-assisted linking/GraphRAG เป็นของระบบเต็ม
